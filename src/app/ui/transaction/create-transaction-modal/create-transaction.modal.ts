...
import { CreateTransactionModalPayload } from './create-transaction.modal.payload';
...
export class CreateTransactionModal
  extends ModalContentBaseComponent<
    CreateTransactionModalPayload,
    Transaction | null
  >
  implements OnInit
{
  ...
  async onFormSubmit(payload: CreateTransactionPayload): Promise<void> {
    this.errors.set([]);
    const resp = await this.transactionService.create(payload);
